...
export const generateImageAI = async (concept: ConceptData,text: MeditationScript): Promise<string> => {
  try {
    ...
    - 사실적이게 표현을해야합니다.
    - 사람이 있는 이미지는 생성하지 마세요.

    이미지 생성 시작해주세요.
    `;

    let tt = `지금 당신이 서 있는 곳은 쏟아지는 폭포 아래, 세상의 모든 소음이 씻겨 내려가는 고요한 공간입니다. 차가운 물방울이 얼굴을 스치며 일상의 무거운 짐을 잠시 내려놓게 합니다. 귀를 기울여 보세요. 물이 바위에 부딪히는 부드러운 속삭임만이 들려옵니다. 이 순간, 당신의 마음속 모든 걱정과 불안도 물줄기처럼 흘러가도록 허락하세요. 깊은 숨을 들이쉬고, 평온이 당신의 모든 세포를 채우는 것을 느껴보세요. 이제 당신은 온전히 평화롭고 고요합니다.
    영상의 시작과 끝이 이어져야합니다.
    이 문장에 맞게 비디오를 생성하세요`;
